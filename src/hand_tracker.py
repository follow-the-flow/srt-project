...
from src.utils import calculate_joint_angles, map_arm_to_qarm
...
class PoseTracker:
    ...
    def __init__(self, static_image_mode=False, model_complexity=1, min_detection_confidence=0.5):
        ...
        self.model = YOLO('data/models/yolov8n-pose.pt')  # Auto-download model
    ...
    def _draw_coordinate_system(self, img, origin, scale=25, label=""):
        """
        在图像上画3D物理坐标系（与QArm 3D坐标系一致，样式相同）
        ...
        # 绘制样式与3D视图一致：粗箭头 + 粗体标签
        line_thickness = 3
        font_scale = 0.6
        font_thickness = 3

        # X轴 - 红色 - 水平向右
        cv2.arrowedLine(img, (x, y), (x + scale, y),
                       (0, 0, 255), line_thickness, tipLength=0.25)
        cv2.putText(img, "X", (x + scale + 8, y + 8),
                   cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 255), font_thickness)

        # Y轴 - 绿色 - 向下（前方远离相机）
        cv2.arrowedLine(img, (x, y), (x, y + scale),
                       (0, 255, 0), line_thickness, tipLength=0.25)
        cv2.putText(img, "Y", (x - 5, y + scale + 12),
                   cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 255, 0), font_thickness)

        # Z轴 - 蓝色 - 向上（垂直向上）
        cv2.arrowedLine(img, (x, y), (x, y - scale),
                       (255, 0, 0), line_thickness, tipLength=0.25)
        cv2.putText(img, "Z", (x - 8, y - scale - 8),
