...
from src.hand_tracker import PoseTracker, get_camera_stream
from src.qarm_sim import QarmSimulator
...
class HandControlQarmDemo:
    ...
    def _init_3d_objects(self):
        ...
        self.scatters_3d = {}
        self.titles_3d = {}
        self.quivers_3d = {}  # Store coordinate axis arrows
        ...
        for view_name, ax in self.axes_3d.items():
            ...
            ax.quiver(origin[0], origin[1], origin[2],
                     0, arrow_length, 0,
                     color='green', arrow_length_ratio=0.3, linewidth=2.5)
            # Z轴箭头 (蓝色)
            ax.quiver(origin[0], origin[1], origin[2],
                     0, 0, arrow_length,
            ...
            # 添加轴标签
            ax.text(arrow_length * 1.2, 0, 0, 'X', color='red', fontsize=10, fontweight='bold')
            ax.text(0, arrow_length * 1.5, 0, 'Y', color='green', fontsize=10, fontweight='bold')
            ax.text(0, 0, arrow_length * 1.2, 'Z', color='blue', fontsize=10, fontweight='bold')
    ...
    def process_camera(self):
        ...
        if result['detected']:
            from src.utils import map_arm_to_qarm
